...
public class LogFileWriter {
    ...
    private HashMap<String, BufferedWriter> openTextFileList = new HashMap<>();
    private HashMap<String, BufferedOutputStream> openBinaryFileList = new HashMap<>();
    ...
    BufferedWriter getTextFile(String filename) {
        ...
        try {
            if (openTextFileList.containsKey(filename)) {
                return openTextFileList.get(filename);
            }
            ...
            openTextFileList.put(filename, writer);
    ...
    BufferedOutputStream getBinaryFile(String filename) {
        ...
        try {
            if (openBinaryFileList.containsKey(filename)) {
                return openBinaryFileList.get(filename);
            }
            ...
            openBinaryFileList.put(filename, writer);
    ...
    public void flush() {
        try {
            for (var writer : openTextFileList.values()) {
                writer.flush();
            }
            for (var writer : openBinaryFileList.values()) {
                writer.flush();
            }
        } catch (IOException e) {
            e.printStackTrace();
